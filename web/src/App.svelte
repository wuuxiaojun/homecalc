        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-800/60 pb-4">
          <div>
            <div class="flex items-center gap-2">
              <span class="text-xs font-mono px-2 py-0.5 rounded font-semibold {appState.activeSlot.purchase ? 'bg-emerald-950/80 text-emerald-300 border border-emerald-800/60' : 'bg-zinc-900 text-zinc-400 border border-zinc-800'}">
                Slot {appState.activeSlotId}
              </span>
              <h1 class="text-xl font-bold text-white tracking-tight">
                {appState.activeSlot.purchase?.name || `Slot ${appState.activeSlotId} (Empty)`}
              </h1>
            </div>
